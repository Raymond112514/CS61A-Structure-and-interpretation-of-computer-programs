...
def pick(paragraphs, select, k):
    ...
    ''
    """
    i, count, returned = 0, -1, ''

    while i <= len(paragraphs) - 1:
        if select(paragraphs[i]):
            count += 1
            returned = paragraphs[i]
            if count == k:
                return returned
        i += 1
    return ''
...
def about(topic):
    ...
    def select(paragraphs):
        ...
        for text_in_topic in topic:
            for text_in_paragraphs in paragraphs:
                if text_in_topic == text_in_paragraphs:
                    return True
        return False
    return select
...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0:
        if len(source_words) == 0 or typed_words == source_words[0]:
            return 100.0
        else:
            return 0.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(0,len(word_list)):
        ...
        """If typed_word is in word_list, then return typed_word"""
        
        if typed_word == word_list[i]:
            return typed_word
        
        """Construct a vector of difference values"""
        
        difference.append(diff_function(typed_word,word_list[i],limit))
...
def time_per_word(words, times_per_player):
    ...
    for j in times_per_player:
            times.append([j[i+1]-j[i] for i in range(0,len(j)-1)])
...
def fastest_words(match):
    ...
    for j in words:
        ...
        minimum = times[0][words.index(j)]
            
        for m in times:
            if m[words.index(j)] < minimum:
                index_tracker = times.index(m)
                minimum = times[index_tracker][words.index(j)]
                
        fastest[index_tracker].append(j)
        index_tracker = 0
