...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while i <= 10:
        ...
        testing = average_roll_dice(i,dice)

        if testing > winning:

            winning = testing
            maximum_scoring_num = i

        i += 1

    return maximum_scoring_num
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    test_threshold = tail_points(opponent_score)
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    piggy_tail = score + tail_points(opponent_score)

    if perfect_square(piggy_tail):
        square_swine = next_perfect_square(piggy_tail)
    else:
        square_swine = piggy_tail

    if square_swine >= score + threshold:
        return 0
    else:
        return num_rolls
